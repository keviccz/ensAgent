...
# Parse arguments
option_list <- list(
  make_option(c("--spatial_data"), type="character", help="Path to spatial count matrix RDS"),
  make_option(c("--spatial_loc"), type="character", help="Path to spatial location RDS"),
  make_option(c("--sc_data"), type="character", help="Path to scRNA-seq reference RDS"),
...
# Load spatial data
cat("[IRIS] Loading spatial data...\n")
sp_in <- readRDS(args$spatial_data)
...
cat("[IRIS] Loading scRNA-seq reference...\n")
sc_in <- readRDS(args$sc_data)
